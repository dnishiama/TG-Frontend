<template>
  <div class="card-body">
    ...
    <div id="divListar">
      <div class="col-lg-12">
        <h2>Listar</h2>Clique no botão para visualizar os departamentos cadastrados
        <button
          v-on:click="buscarTodos"
          class="glyphicon glyphicon-plus"
        ></button>
      </div>
      <table id="tabDepartamento" hidden="true" class="table table-striped">
        <thead>
          <tr>
            <th>Id</th>
            <th>Campus</th>
...
export default {
  ...
  methods: {
    buscarTodos() {
      var show = document.getElementById("tabDepartamento");
      if (show.hidden === true) {
        show.hidden = false;
      }
      axios
